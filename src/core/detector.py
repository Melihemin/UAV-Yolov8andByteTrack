...
class Detector:
    ...
    def detect(self, frame, conf_thresh=0.25):
        ...
        """
        # Run tracking
        tracker_path = os.path.join(config.BASE_DIR, 'config/bytetrack.yaml')
        results = self.model.track(frame, conf=conf_thresh, tracker=tracker_path, persist=True, verbose=False)
